...
for ii = 1:length(ul_step)
    ...
    fname = strcat('sucrose_10_', num2str(ul_step(ii)),'00ul_sucrose_40_Lower.txt');
...
figure
plotyy(Wavelength(Locations), Sensitivity, Wavelength(Locations), R2)
xlabel('Wavelength');
...
if(sum(IND)>0)
    ...
    fprintf('Sensitivity = %f dB/RIU\n', Sensitivity_ok(Imax));
    fprintf('R2 = %f\n', R2_ok (Imax));
    fprintf('At the wavelength %f nm\n', Wavelength_ok(Imax));
    fprintf('Corresponding to the index %f\n\n\n', Locations_ok(Imax));
